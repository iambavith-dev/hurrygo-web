    ...
    return (
        <div className="hero w-full py-14 flex flex-col items-center" >
            <div className="max-w-screen-lg w-full flex flex-col items-center justify-center gap-2" >
                ...
                <h1 className="text-2xl font-medium flex flex-row items-center gap-2" >What Our <span className="text-blue-500" >Customer</span> Say</h1>

                <div className="grid grid-cols-2 px-4 lg:grid-cols-3 gap-4 mt-6" >
                    {
                        allData?.map((item, index) => {
                            return (
                                <div key={`testimonials_${index}`} className="p-3 border border-dashed hover border-slate-200 rounded-2xl bg-white shadow-2xl shadow-slate-100 flex flex-col gap-1 hover:shadow-slate-300 transition-all duration-300" >
